function [error_E, error_R_cov_E, error_sf] = ...
    ...
    signal_target_list, inter_sig_list, sig_length, SNR_dB_list, N_I, ...
...
% for different SNR values, for different signals in each monte carlo iteration,
% random desired source angles
    ...
    fontSize = 20;
    error_E = zeros(length(SNR_dB_list), monte_carlo_num);
    error_R_cov_E = zeros(length(SNR_dB_list), monte_carlo_num);
    error_sf = zeros(length(SNR_dB_list), monte_carlo_num);
    dSIR_E = zeros(length(SNR_dB_list), monte_carlo_num);
    dSIR_R_cov_E = zeros(length(SNR_dB_list), monte_carlo_num);
    dSIR_sf = zeros(length(SNR_dB_list), monte_carlo_num);
    dSNR_E = zeros(length(SNR_dB_list), monte_carlo_num);
    dSNR_R_cov_E = zeros(length(SNR_dB_list), monte_carlo_num);
    dSNR_sf = zeros(length(SNR_dB_list), monte_carlo_num);
    ...
    for i_i = 1 : N_I
        ...
        [~, h_inter(i_i, :, :)] = evalc(['rir_generator(c, fs, mics_pos_mat, inter_pos(i_i, :), room_dims, ', ...
            'beta, filter_samples, mtype, order, dim, orientation, hp_filter)']);
    end
    ...
    for monte_carlo_index = 1 : monte_carlo_num
        ...
        for i_i = 1 : N_I
            inter_sig_padded(i_i, :) = [squeeze(inter_sig_list(i_i, monte_carlo_index, :)).' ...
                zeros(1, filter_samples)];
        end
        ...
        for SNR_index = 1 : length(SNR_dB_list)
            ...
            for mic_index = 1 : num_of_mics
                noise_gain(mic_index) = norm(mics_sig(mic_index, :)) / 10^(SNR_dB_list(SNR_index) / 20);
            ...
            dSIR_E(SNR_index, monte_carlo_index) = SIR_E - SIR_dB_eff;
            dSIR_R_cov_E(SNR_index, monte_carlo_index) = SIR_R_cov_E - SIR_dB_eff;
            dSIR_sf(SNR_index, monte_carlo_index) = SIR_sf - SIR_dB_eff;
            dSNR_E(SNR_index, monte_carlo_index) = SNR_E - SNR_dB_list(SNR_index);
            dSNR_R_cov_E(SNR_index, monte_carlo_index) = SNR_R_cov_E - SNR_dB_list(SNR_index);
            dSNR_sf(SNR_index, monte_carlo_index) = SNR_sf - SNR_dB_list(SNR_index);
            ...
            error_sf_cur = norm(estimated_sig_sf_partial - ...
                first_mics_sig_partial)^2 / first_mics_sig_partial_norm;
            error_E(SNR_index, monte_carlo_index) = 10 * log10(error_E_cur);
            error_R_cov_E(SNR_index, monte_carlo_index) = 10 * log10(error_R_cov_E_cur);
            error_sf(SNR_index, monte_carlo_index) = 10 * log10(error_sf_cur);
        end
    end
    ...
    mean_error_comb_freq_est = mean(error_sf, 2);
    std_error_comb_freq_est = std(error_sf, 0, 2);
    ...
    grid on
    grid minor
    errorbar(SNR_dB_list, mean_error_E, std_error_E, ':', 'LineWidth', 2.5, ...
        'Color', [0.47,0.67,0.19])
    errorbar(SNR_dB_list, mean_error_R_cov_E, std_error_R_cov_E, '--', 'LineWidth', 2.5, ...
        'Color', [0.85 0.33 0.10])
    errorbar(SNR_dB_list, mean_error_comb_freq_est, std_error_comb_freq_est, '-', 'LineWidth', 2.5, ...
    ...
    leg.FontSize = 18;
    hold off
    xlim([SNR_dB_list(1) SNR_dB_list(end)]);
    ...
    grid on
    grid minor
    errorbar(SNR_dB_list, mean_dSIR_E, std_dSIR_E, ':', 'LineWidth', 2.5, ...
        'Color', [0.47,0.67,0.19])
    errorbar(SNR_dB_list, mean_dSIR_R_cov_E, std_dSIR_R_cov_E, '--', 'LineWidth', 2.5, ...
        'Color', [0.85 0.33 0.10])
    errorbar(SNR_dB_list, mean_dSIR_sf, std_dSIR_sf, '-', 'LineWidth', 2.5, ...
    'Color', [0 0.4470 0.7410])
    ax = gca;
    ax.FontSize = 20;
    ...
    grid on
    grid minor
    errorbar(SNR_dB_list, mean_dSNR_E, std_dSNR_E, ':', 'LineWidth', 2.5, ...
        'Color', [0.47,0.67,0.19])
    errorbar(SNR_dB_list, mean_dSNR_R_cov_E, std_dSNR_R_cov_E, '--', 'LineWidth', 2.5, ...
        'Color', [0.85 0.33 0.10])
    errorbar(SNR_dB_list, mean_dSNR_sf, std_dSNR_sf, '-', 'LineWidth', 2.5, ...
    'Color', [0 0.4470 0.7410])
    ax = gca;
    ax.FontSize = 20;
